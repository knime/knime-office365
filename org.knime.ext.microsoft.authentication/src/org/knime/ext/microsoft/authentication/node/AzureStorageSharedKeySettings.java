 ...
 *
 * History
 *   2023-09-28 (Zkriya Rakhimberdiyev, Redfield SE): created
 */
...
/**
 * Implementation of {@link DefaultNodeSettings} to specify Azure Storage shared
 * key.
 *
 * @author Zkriya Rakhimberdiyev, Redfield SE
 */
@SuppressWarnings("restriction")
public class AzureStorageSharedKeySettings implements CredentialsSettings {

    /**
     * The name of the Credentials flow variable.
     */
    @Widget(title = "Storage account name and shared/access key (flow variable)", //
            description = """
                    Specifies the credentials flow variable with the storage account name
                    and shared key (also called access key) to use.
                    """)
    ...
    public void validateOnConfigure(final CredentialsProvider credsProvider) throws InvalidSettingsException {
        ...
        if (retrieve(credsProvider).isPresent()) {
            validateLogin(credsProvider, "Storage account name is required");
            validateSecret(credsProvider, "Access/shared key is required");
        }
    }
