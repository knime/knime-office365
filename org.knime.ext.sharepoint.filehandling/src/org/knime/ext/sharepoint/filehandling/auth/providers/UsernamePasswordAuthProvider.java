...
import org.knime.ext.sharepoint.filehandling.auth.providers.ui.UsernamePasswordProviderEditor;
...
/**
 * {@link AbstractAuthProvider} implementation that performs authentication
 * using username and password provided by user.
 *
 * @author Alexander Bondaletov
 */
public class UsernamePasswordAuthProvider extends AbstractAuthProvider {
     ...
     */
    public UsernamePasswordAuthProvider(final AzureConnection connection) {
        super(connection);
    ...
    public void validateSettings(final ConfigRO settings) throws InvalidSettingsException {
        UsernamePasswordAuthProvider temp = new UsernamePasswordAuthProvider(m_connection);
    ...
    protected CompletableFuture<IAuthenticationResult> acquireToken(final PublicClientApplication app) {
        ...
        return app.acquireToken(
                UserNamePasswordParameters.builder(m_connection.getScopes(), username, password.toCharArray()).build());
    }
}
