...
/**
 * Base class for editor component for {@link AbstractAuthProvider}.
 *
 *
 * @param <T>
 ...
 */
public abstract class AbstractAuthProviderEditor<T extends AbstractAuthProvider> extends JPanel {
    private static final long serialVersionUID = 1L;

    /**
     * {@link AbstractAuthProvider} instance.
     */
     ...
     *
     */
    public AbstractAuthProviderEditor(final T provider) {
        m_provider = provider;
        initUI();
        m_provider.getConnection().getTokenCacheModel().addChangeListener(e -> updateStatusLabel(false));
    }
    ...
    private void onLogin() {
        m_worker = new SwingWorkerWithContext<Void, Void>() {
            ...
            protected Void doInBackgroundWithContext() throws Exception {
                m_provider.validate();
                m_provider.getConnection().validate();
                m_provider.login();
                return null;
            }
    ...
    private void updateStatusLabel(final boolean inProgress) {
        if (inProgress) {
            ...
            boolean loggedIn = m_provider.getConnection().isLoggedIn();
