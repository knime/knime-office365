...
public final class SharePointListUtils {
    ...
    private static final String SELECT_NAME_DISPLAY_NAME_ID = "name,displayName,id";
    ...
    public static Optional<String> getListIdByInternalName(final GraphServiceClient<Request> client, //
        ...
        return getListIdByFilter(client, siteId, l -> l.name.equals(internalName), "name,id", null);
    }
    ...
    /**
     * Get a list by name by checking which list internal name matches the given
     * name, if no such list is found the display name will be used instead.
     *
     * @param client
     *            the {@link GraphServiceClient}
     * @param siteId
    ...
    private static Optional<String> getListIdByFilter(final GraphServiceClient<Request> client, final String siteId,
        ...
        try {
            ...
            while (nextRequest != null) {
                final var resp = nextRequest.buildRequest(java.util.List.of(new QueryOption("select", select))).get();
    ...
    private static Optional<String> getListIdByFilter(final GraphServiceClient<Request> client, final String siteId,
        ...
        try {
            return client.sites(siteId).lists().buildRequest(getFilter(property, value)) //
                    .get().getCurrentPage().stream().filter(l -> l.system == null).findFirst().map(l -> l.id);
        } catch (GraphServiceException ex) {
            throw new IOException("Error during list ID retrieval: " + ex.getServiceError().message, ex);
        }
    }
    ...
    private static java.util.List<Option> getFilter(final String key, final String value) {
        return java.util.List.of( //
                new QueryOption("select", "system,id," + key), //
