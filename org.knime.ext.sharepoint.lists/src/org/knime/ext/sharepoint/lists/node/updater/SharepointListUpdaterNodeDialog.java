...
/**
 * SharePoint Online List Updater node implementation of a
 * {@link NodeDialogPane}.
 *
 * @author Jannik Löscher, KNIME GmbH, Konstanz, Germany
 */
public final class SharepointListUpdaterNodeDialog extends NodeDialogPane {

    private static final NodeLogger LOGGER = NodeLogger.getLogger(SharepointListUpdaterNodeDialog.class);

    private final SharepointListUpdaterConfig m_config;

    private final SharepointListSettingsPanel m_listSettingsPanel;

    private final DialogComponentColumnNameSelection m_idColumn;

    private final TimeoutPanel m_timeoutPanel;

    @SuppressWarnings("unchecked")
    SharepointListUpdaterNodeDialog() {
        m_config = new SharepointListUpdaterConfig();
        m_listSettingsPanel = new SharepointListSettingsPanel(m_config.getSharepointListSettings(), false, false);
        m_idColumn = new DialogComponentColumnNameSelection(m_config.getIdColNameModel(), "ID column:", 1,
                StringValue.class);
    ...
    private JPanel createSettingsPanel() {
        ...
        panel.add(m_idColumn.getComponentPanel(), gbc.resetX().incY().weight(1, 0).fillHorizontal().build());
    ...
    protected void loadSettingsFrom(final NodeSettingsRO settings, final PortObjectSpec[] specs)
        ...
        try {
            ...
            m_idColumn.loadSettingsFrom(settings, specs);
        } catch (InvalidSettingsException ex) {
            LOGGER.error("An unexpected error occured during the loading of the settings.", ex);
        }

        m_listSettingsPanel.settingsLoaded((CredentialPortObjectSpec) specs[0]);
    }
